...
STAT_MAP_FILE = os.path.join('data/ipastatmap.json')
...
def frequency_table( count_table, threshold=0.05 ):
    ...
    result = {}
    for (ipa,counts) in list(count_table.items()):
...
def print_frequency_table( table ):
    ...
    for ipa,possible in sorted(table.items()):
        ...
        for (left,right) in zip(lefts,possible):
            print(('%s\t%s (%0.1f))'%(left,right[0],right[1]*100)))
...
#                continue
        ...
        table = json.dumps( {
            'lines': i,
            'mapping': frequency_table(mapping),
    ...
    try:
        for i,line in enumerate(open(dictionary)):
            ...
            try:
                ...
                print((
                    'Failed to translate: %s (%s) -> %s  char=%s(%r)'%(
                    word,
                    err.args[1],
                    description,
